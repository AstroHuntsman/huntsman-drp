...
class ButlerRepository(HuntsmanBase):
    ...
    def get_filename(self, data_type, data_id):
        ...
        Returns:
            str: The filename.
        """
        filename = self.butler.get(f"{data_type}_filename", data_id)[0]
    ...
    def _setup_rerun(self, rerun):
        """ Setup the rerun by making necessary directories.
        Args:
            rerun (str): The rerun name.
        """
        rerundir = os.path.join(self.butler_directory, "rerun", rerun)
        os.makedirs(rerundir, exist_ok=True)

...
def solve_field_task(options, fpath):
    ...
    if fpath.endswith('.fz'):
        ...
        # Just incase the previous except doesn't capture an fpack error, check if the expected output file exists
        if not os.path.isfile(fpath):
    ...
    # is attempt times out, log the info
        ...
        debug_logger.warning(f"get_solve_field() Timed out on {fpath} with error: \n{e}\n")
